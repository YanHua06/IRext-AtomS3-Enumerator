...
static int  g_testSwing    = 0;      // 0=关, 1=开
static int  g_testAcKey    = 0;      // AC 功能码: 0=电源/全帧, 9=风速, 10=扫风, 11=固定风向
...
bool irSend() {
  ...
  if (g_testIsAc) {
    ...
    rawLen = ir_decode(g_testAcKey, g_rawBuf, &ac);
  } else {
...
void printStatus() {
  ...
  if (g_testIsAc) termPrint("  空调参数: temp=%d mode=%d fan=%d swing=%d key=%d", g_testTemp, g_testMode, g_testFan, g_testSwing, g_testAcKey);
...
void printHelp() {
  ...
  termPrint("║ setac <t> <m> <f> [sw] [key] 设空调参数 ║");
  termPrint("║   t=温度 m=模式 f=风速 sw=扫风 key=功能码║");
  termPrint("║   key: 0=全帧/电源 9=风速 10=扫风 11=定向║");
  termPrint("║ fanup          风速+ (key=9)           ║");
  termPrint("║ fandown        风速- (key=9)           ║");
  termPrint("║ swingon        扫风开 (key=10)         ║");
  termPrint("║ swingoff       扫风关 (key=10)         ║");
  termPrint("║ fix            固定风向 (key=11)       ║");
...
void handleCommand(const String& rawCmd) {
  ...
  if (cmd == "start") {
    ...
    // setac <temp> <mode> <fan> [swing] [key]
    int t=26, m=0, f=0, sw=0, key=0;
    sscanf(cmd.c_str(), "setac %d %d %d %d %d", &t, &m, &f, &sw, &key);
    g_testIsAc = true;
    g_testTemp = t; g_testMode = m; g_testFan = f; g_testSwing = sw; g_testAcKey = key;
    termPrint("[*] 切换为空调模式 temp=%d mode=%d fan=%d swing=%d key=%d", t, m, f, sw, key);
  } else if (cmd == "fanup") {
    g_testIsAc = true;
    g_testAcKey = 9;  // AC_FUNCTION_WIND_SPEED
    g_testFan = (g_testFan + 1) % 4;
    termPrint("[*] 风速+ → fan=%d key=9", g_testFan);
    irSend();
  } else if (cmd == "fandown") {
    g_testIsAc = true;
    g_testAcKey = 9;  // AC_FUNCTION_WIND_SPEED
    g_testFan = (g_testFan + 3) % 4;
    termPrint("[*] 风速- → fan=%d key=9", g_testFan);
    irSend();
  } else if (cmd == "swingon") {
    g_testIsAc = true;
    g_testAcKey = 10;  // AC_FUNCTION_WIND_SWING
    g_testSwing = 1;
    termPrint("[*] 扫风开 → swing=1 key=10");
    irSend();
  } else if (cmd == "swingoff") {
    g_testIsAc = true;
    g_testAcKey = 10;  // AC_FUNCTION_WIND_SWING
    g_testSwing = 0;
    termPrint("[*] 扫风关 → swing=0 key=10");
    irSend();
  } else if (cmd == "fix") {
    g_testIsAc = true;
    g_testAcKey = 11;  // AC_FUNCTION_WIND_FIX
    termPrint("[*] 固定风向 key=11");
    irSend();
...
<div id="shortcuts">
  ...
  <button class="sc" onclick="send('fanup')">💨 fan+</button>
  <button class="sc" onclick="send('fandown')">💨 fan-</button>
  <button class="sc" onclick="send('swingon')">🔄 sw on</button>
  <button class="sc" onclick="send('swingoff')">🔄 sw off</button>
  <button class="sc" onclick="send('fix')">📍 fix</button>
</div>
<div id="term"></div>
<div id="inputrow">
